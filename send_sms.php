...
CONCAT(CASE WHEN a.reason is not null THEN 'REVISI ORDER|' ELSE '' END, 'R:', rtrim(b.name) ,'|', 'D:', rtrim(b1.name) ,'|','Kode: ', a.code_booking, '|', GROUP_CONCAT(CONCAT(d.`name`,' =', c.qty,'', e.name) SEPARATOR '|'))
    AS message
FROM `orderretailtable` a 
JOIN `retailtable` b ON b.`id`=a.`id_retail` 
JOIN `distributortable` b1 ON b1.`id`=a.`id_distributor`
...
SELECT a.`id`, 'retailer_order' AS order_from, 'orderretailtable' AS table_order, b1.`telephone` AS detination, 'system' AS `sent_from`,
CONCAT(CASE WHEN a.reason is not null THEN 'REVISI ORDER|' ELSE '' END, 'R:', rtrim(b.name) ,'|', 'D:', rtrim(b1.name) ,'|','Kode: ', a.code_booking, '|', GROUP_CONCAT(CONCAT(d.`name`,' =', c.qty,'', e.name)  SEPARATOR '|'))
    AS message
FROM `orderretailtable` a 
JOIN `retailtable` b ON b.`id`=a.`id_retail` 
...
CONCAT(CASE WHEN a.reason is not null THEN 'REVISI ORDER|' ELSE '' END, 'P:', rtrim(b.name) ,'|', 'R:', rtrim(b1.name) ,'|','Kode: ', a.code_booking, '|', GROUP_CONCAT(CONCAT(d.`name`,' =', c.qty,'', e.name) SEPARATOR '|'))
    AS message
FROM `ordertable` a 
JOIN `custtable` b ON b.id=a.id_customer
JOIN `retailtable` b1 ON b1.id=a.id_retail
...
SELECT a.`id`, 'customer_order' AS order_from, 'ordertable' AS table_order, b1.`telephone` AS detination, 'system' AS `sent_from`,
CONCAT(CASE WHEN a.reason is not null THEN 'REVISI ORDER|' ELSE '' END, 'P:', rtrim(b.name) ,'|', 'R:', rtrim(b1.name) ,'|','Kode: ', a.code_booking, '|', GROUP_CONCAT(CONCAT(d.`name`,' =', c.qty,'', e.name)  SEPARATOR '|'))
    AS message
FROM `ordertable` a 
JOIN `custtable` b ON b.id=a.id_customer
